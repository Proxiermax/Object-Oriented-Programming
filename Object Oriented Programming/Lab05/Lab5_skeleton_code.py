...
def search_subject_by_id(subject_id):
    return [subject for subject in subject_list if subject_id == subject.get_subject_id() and type(subject_id) == str]

# TODO 2 : function สำหรับค้นหา instance ของนักศึกษาใน student_list
def search_student_by_id(student_id):
    return [student for student in student_list if student_id == student.get_student_id() and type(student_id) == str]
...
def search_student_enroll_in_subject(subject):
    if isinstance(subject, Subject):
        return [enrollment for enrollment in enrollment_list if enrollment.get_subject() == subject]
    else:
        return 'Error'
    # return ([enrollment for enrollment in enrollment_list if enrollment.get_subject() == subject] if isinstance(subject, Subject) else None)
...
def get_teacher_teach(subject_search):
    if isinstance(subject_search, Subject):
        for enrollment in enrollment_list:
            if enrollment.get_subject() == subject_search:
                return enrollment.get_subject().get_teacher()
    else:
        return 'Not Found'
...
def get_student_GPS(student):
    if isinstance(student, Student):
        total_score = sum(grade_to_count(enrollment.get_grade()) * enrollment.get_subject().get_credit() for enrollment in enrollment_list if enrollment.get_student() == student)
        total_credit = sum(enrollment.get_subject().get_credit() for enrollment in enrollment_list if enrollment.get_student() == student)
...
# print(enrollment_list[0].get_student().get_student_id(), enrollment_list[0].get_subject().get_subject_id())
# print(enroll_to_subject(student_list[0], subject_list[0]))
# print(drop_from_subject(student_list[0], subject_list[0]))
# print(enroll_to_subject(student_list[0], subject_list[0]))
